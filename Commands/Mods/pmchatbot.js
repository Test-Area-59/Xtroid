...
module.exports = {
  ...
  start: async (
    ...
    if (args[0] === "on") {
      if (!checkdata) {
        await new xchar({ id: "1", PMchatBot: "true" }).save();
        Xtroid.sendMessage(
          m.from,
          {
        ...
        await xchar.updateOne({ id: "1" }, { PMchatBot: "true" });
        return Xtroid.sendMessage(
          m.from,
          { text: `*PM Chatbot Activated !*` },
          { quoted: m }
      ...
      if (!checkdata) {
        await new xchar({ id: "1", PMchatBot: "false" }).save();
        return Xtroid.sendMessage(
          m.from,
        ...
        await xchar.updateOne({ id: "1" }, { PMchatBot: "false" });
        return Xtroid.sendMessage(
          m.from,
