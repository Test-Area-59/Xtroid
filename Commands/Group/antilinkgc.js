...
module.exports = {
    ...
    start: async (
      ...
      if (args[0] === "on") {
        if (!checkdata) {
          await new xt({ id: m.from, antilink: "true" }).save();
          Xtroid.sendMessage(
            m.from,
            {
          ...
          await xt.updateOne({ id: m.from }, { antilink: "true" });
          return Xtroid.sendMessage(
            m.from,
        ...
        if (!checkdata) {
          await new xt({ id: m.from, antilink: "false" }).save();
          return Xtroid.sendMessage(
            m.from,
          ...
          await xt.updateOne({ id: m.from }, { antilink: "false" });
          return Xtroid.sendMessage(
            m.from,
