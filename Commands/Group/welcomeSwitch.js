...
module.exports = {
  ...
  start: async (
    ...
    if (args[0] === "on") {
      if (!checkdata) {
        await new xt({ id: m.from, switchWelcome: "true" }).save();
        Xtroid.sendMessage(
          m.from,
          {
        ...
        await xt.updateOne({ id: m.from }, { switchWelcome: "true" });
        return Xtroid.sendMessage(
          m.from,
          {
            text: `*Welcome/Goodbye* messages has been *Activated* in this group!`,
      ...
      if (!checkdata) {
        await new xt({ id: m.from, switchWelcome: "false" }).save();
        return Xtroid.sendMessage(
          m.from,
          {
        ...
        await xt.updateOne({ id: m.from }, { switchWelcome: "false" });
        return Xtroid.sendMessage(
          m.from,
          {
            text: `*Welcome/Goodbye* messages has been *De-Activated* in this group!`,
