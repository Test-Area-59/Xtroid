...
module.exports = {
  ...
  start: async (
    ...
    if (args[0] === "on") {
      if (!checkdata) {
        await new xt({ id: m.from, botSwitch: "true" }).save();
        Xtroid.sendMessage(
          m.from,
          {
        ...
        await xt.updateOne({ id: m.from }, { botSwitch: "true" });
        return Xtroid.sendMessage(
          m.from,
      ...
      if (!checkdata) {
        await new xt({ id: m.from, botSwitch: "false" }).save();
        return Xtroid.sendMessage(
          m.from,
          {
        ...
        await xt.updateOne({ id: m.from }, { botSwitch: "false" });
        return Xtroid.sendMessage(
          m.from,
          {
            text: `${botName} has been De-Activated in this group !\n\nNow only *Admins* can use bot`,
