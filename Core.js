...
const { xt, xtu, xchar } = require("./Database/dataschema.js");
...
module.exports = async (Xtroid, m, commands, chatUpdate, store) => {
  try {
    ...
    global.suppL = "https://t.me/nxtbots";
    ...
    let character = await xchar.findOne({
      id: "1",
    });
    if (character) {
      CharacterSelection = character.seletedCharacter;
    } else {
      let newCharacter = new xchar({
        id: "1",
        seletedCharacter: "0",
      });
    ...
    let checkdata = await xt.findOne({
      id: m.from,
    });
    if (!checkdata) {
      let newdata = new xt({
    ...
    let modSTATUS = await xtu.findOne({
      id: m.sender,
    });
    var modStatus = "false";
    if (!modSTATUS) {
      await xtu.create({ id: m.sender, addedMods: "false" });
    ...
    let botModeSet = await xchar.findOne({
      id: "1",
    });
    ...
    let botSwitchGC = await xt.findOne({
      id: m.from,
    });
    ...
    let chatbotStatus = await xt.findOne({
      id: m.from,
    });
    var csts = "false";
    if (chatbotStatus) {
    ...
    let PMchatBotStatus = await xchar.findOne({
      id: "1",
    });
    var PMcsts = "false";
    if (PMchatBotStatus) {
    ...
    let nsfwstatus = await xt.findOne({
      id: m.from,
    });
    ...
    let banGCStatus = await xt.findOne({ id: m.from });
    ...
    if (body.startsWith(prefix) && !icmd) {
      let mikutext = `No such command programmed *${pushname}* ! Type *${prefix}help* or press the button below to get my full command list!\n`;
